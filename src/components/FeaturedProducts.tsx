...
const FeaturedProducts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      console.log('FeaturedProducts: All products:', fetchedProducts.length);
      const featuredProducts = fetchedProducts.filter(p => p.main_tag === 'Featured').slice(0, 4);
      console.log('FeaturedProducts: Featured products found:', featuredProducts.length);
        ...
        <div className="text-center mb-12">
          ...
          {products.length === 0 && (
            <p className="text-yellow-400 text-sm mt-2">
              No featured products found. Make sure to add products with "Featured" tag.
            </p>
          )}
        </div>
